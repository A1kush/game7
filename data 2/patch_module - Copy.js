...
(function(global){
  ...
  Patch.injectButtons = function(){
    ...
    if (!header || document.getElementById('btnAutoSell')) return;
    const ctn = document.createElement('span'); ctn.style.marginLeft='8px';
    ctn.innerHTML = `
      <button id="btnAutoSell" class="btn">Auto Sell</button>
      <button id="btnOpenAll" class="btn">Open All Gifts</button>
      <button id="btnAutoEquipAll" class="btn">Auto Equip (All)</button>
    `;
    header.appendChild(ctn);
    document.getElementById('btnAutoSell').onclick = Patch.autoSellAll;
    document.getElementById('btnOpenAll').onclick = Patch.openAllGifts;
    document.getElementById('btnAutoEquipAll').onclick = Patch.autoEquipAll;
  };
